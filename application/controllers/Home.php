...
class Home extends CI_Controller
{
    ...
    public function index()
    {
        ...
        // unset($_SESSION);
        // if (isset($_SESSION['social_error'])) {
        //     echo $_SESSION['social_error'];
        //     unset($_SESSION['social_error']);
        // }
        // if (0) {
        if (isset($_GET["code"])) {
            ...
            if (!isset($token["error"])) {
                ...
                if ($this->ion_auth->login_social($data["email"], "", true)) {
                    //if the login is successful
                    redirect('home', 'refresh');
                } else {
                    $_SESSION['social_error'] = $this->ion_auth->errors().', check';
                    redirect('home');
                }
                $this->session->set_userdata("user_data", $user_data);
            } else {
                $_SESSION['social_error'] = "Somethings wend wrong" . $token["error"];
                redirect('home');
            }
        }
