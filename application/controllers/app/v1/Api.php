  ...
  */
    ...
    public function login()
    {
        ...
        */
        ...
        //     return false;
        // }
        // echo $hashToStoreInDb = password_hash("12345678", PASSWORD_BCRYPT); die;
        ...
        if (
           ...
           if (empty($post['mobile']) && empty($post['email'])) {
               $cuure = 1;
           }
        } else {
            $cuure = 1;
        ...
        if (isset($cuure)) {
            $this->response['error'] = true;
            $this->response['message'] = "Please provide email or password";
            print_r(json_encode($this->response));
            return false;
        }
        ...
        if (isset($post['mobile'])) {
            ...
            $query = $this->db->select('mobile, email, id, password, active, last_login')
            ...
            if ($query->num_rows() === 1) {
                $user = $query->row();
                // print_r($user->mobile); die;
                $mobile = $user->mobile;
                $login = $this->ion_auth->login($mobile, $this->input->post('password'), false);
                $user_data = fetch_details('users', ['mobile' => $mobile]);
        ...
        if ($login) {
            if (!$this->ion_auth->is_admin() && !$this->ion_auth->is_seller() && !$this->ion_auth->is_delivery_boy()) {
                ...
                $data = fetch_details('users', ['mobile' => $mobile]);
    ...
    public function register_user()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            $data = $this->db->select('u.id,u.username,u.firstname,u.lastname,u.email,u.mobile,u.country_code,c.name as city_name,a.name as area_name')->where([$identity_column => $identity])->join('cities c', 'c.id=u.city', 'left')->join('areas a', 'a.city_id=c.id', 'left')->group_by('email')->get('users u')->result_array();

            foreach ($data as $row) {
                $row = output_escaping($row);
                $tempRow['id'] = (isset($row['id']) && !empty($row['id'])) ? $row['id'] : '';
                $tempRow['firstname'] = (isset($row['firstname']) && !empty($row['firstname'])) ? $row['firstname'] : '';
                $tempRow['lastname'] = (isset($row['lastname']) && !empty($row['lastname'])) ? $row['lastname'] : '';
                $tempRow['username'] = (isset($row['username']) && !empty($row['username'])) ? $row['username'] : '';
                $tempRow['email'] = (isset($row['email']) && !empty($row['email'])) ? $row['email'] : '';
                $tempRow['mobile'] = (isset($row['mobile']) && !empty($row['mobile'])) ? $row['mobile'] : '';
